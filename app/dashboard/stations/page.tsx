...
export default function StationsPage() {
  ...
  // 1. AUTO-RUN GPS ON PAGE LOAD (No button click required)
  useEffect(() => {
    if (typeof window !== "undefined" && "geolocation" in navigator) {
      ...
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Add or update live GPS location in station list
          setStations((prev) => [
            {
              id: "my-live-gps",
              ...
              status: "online",
              aqi: 0,
              lastUpdate: "Live Browser GPS",
            },
            ...prev.filter((s) => s.id !== "my-live-gps"),
          ])
        },
        (error) => {
  ...
  const fetchStations = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStations((prev) => {
          const liveGps = prev.find((s) => s.id === "my-live-gps")
          return liveGps ? [liveGps, ...parsed.filter((s) => s.id !== "my-live-gps")] : parsed
        })
      } else {
        ...
        if (!error && data) {
          setStations((prev) => {
            const liveGps = prev.find((s) => s.id === "my-live-gps")
            return liveGps ? [liveGps, ...(data as Station[]).filter((s) => s.id !== "my-live-gps")] : (data as Station[])
          })
        }
      }
    } catch (err) {
  ...
  // Latitude and Longitude for Google Maps Embed
  const lat = userLocation ? userLocation.lat : -1.9441
  const lng = userLocation ? userLocation.lng : 30.0619
    ...
    <div className="flex flex-1 flex-col min-w-0 max-w-full overflow-x-hidden bg-background">
      <div className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-border bg-card p-5 shadow-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{userLocation ? "Location Active" : "Acquiring GPS..."}</span>
              </Button>
          ...
          {/* 2. GOOGLE MAP WITH DIRECT REDIRECTION ON CLICK */}
          <Card className="bg-card border-border shadow-xs">
            <CardHeader className="pb-3">
              <CardTitle className="text-base font-bold text-foreground">Spatial Network Coverage</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="relative w-full h-[350px] overflow-hidden rounded-lg border border-border">
                <iframe
              ...
              {/* Exact Green Text Indicator Below Map */}
